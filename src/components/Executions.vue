<template>
  <div class="executions">
    ...
    <p class="code" v-for="(execution, index) in executions" :key="index" v-show="execution.visible">
      ...
      <img class="animate__animated animate__zoomInUp" :src="execution.img" width="400" height="300" />
    </p>
    <!--进度条-->
    ...
    progressivelyRun (execution, customDuration) {
      return new Promise((resolve) => {
        let now = new Date(),
          ...
          showCode = () => {
            execution.time = execution.time !== undefined ? execution.time : now.toLocaleTimeString()
            execution.duration = execution.duration !== undefined ? execution.duration : duration.toFixed(2)
            execution.visible = true
            resolve()
          }
        setTimeout(showCode, execution.time)
      })
    },
    // 显示进度条
